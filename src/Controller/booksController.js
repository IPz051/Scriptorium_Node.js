const booksModel = require ('../models/booksModel')

module.exports = {
    
    //GET /api/books
    
    index: (req, res) => {
        const books = booksModel.getAllBooks()
        res.json(books)
    },


    //GET /api/books/:id

    show: (req,res) => {
        const {id} = req.params
        const book = booksModel.getBooksById(id)
        if(!book) {
        return res.status(404).json({message: 'Livro não encontrado'})
        }
        res.json(book)

    },

    //POST /api/books
    
    save: (req,res) => {
        const { title , author , quantityAvalible } = req.body
        if (typeof title !== 'string' || typeof author !== 'string' || typeof quantityAvalible !== 'number') {
            return res.status(400).json ({message: 'Campos inválidos.'})
        }
        const newBook = booksModel.createBook(title, author, quantityAvalible)
        res.status(201).json(newBook)
    },



    //PUT /api/books/:id

    update: (req,res) => {
        const { id } = req.params
        const { title, author, quantityAvalible } = req.body

        const fieldsToUpdate = {}
        if (typeof title === 'string') fieldsToUpdate.title = title
        if (typeof author === 'string') fieldsToUpdate.author = author
        if (typeof quantityAvalible === 'number') fieldsToUpdate.quantityAvalible = quantityAvalible

        const updatedBook = booksModel.updateBook(id, fieldsToUpdate)
        return res.status(200).json(updatedBook)
    },



    //DELETE /api/books/:id

    delete: (req,res) => {
        const {id} = req.params
        const deletedBook = booksModel.deleteBook(id)
        res.status(200).json(deletedBook)
    }



}
